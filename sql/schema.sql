...
select
  ...
  bool_or(e.needs_refresher) as needs_refresher
...
create or replace function public.count_refresher_by_course()
returns table(course_name text, needs bigint)
language sql
security definer
...
as $$
  ...
  where e.needs_refresher is true
  group by c.name
  order by c.name;
$$;
...
create policy "managers_self_access" on public.managers
for select using ( auth.uid() = user_id )
with check ( auth.uid() = user_id );

drop policy if exists "ff_read_all" on public.firefighters;
create policy "ff_read_all" on public.firefighters
...
for insert with check (
  exists (select 1 from public.managers m where m.user_id = auth.uid() and m.role = 'creator')
)
for update using (
  exists (select 1 from public.managers m where m.user_id = auth.uid() and m.role = 'creator')
)
...
create policy "courses_write_creator_only" on public.courses
for insert with check (
  exists (select 1 from public.managers m where m.user_id = auth.uid() and m.role = 'creator')
)
for update using (
...
create policy "sessions_write_managers" on public.sessions
for insert with check ( auth.role() = 'authenticated' )
for update using ( auth.role() = 'authenticated' )
for delete using (
  exists (select 1 from public.managers m where m.user_id = auth.uid() and m.role in ('creator'))
);

drop policy if exists "enrolments_read_all" on public.enrolments;
create policy "enrolments_read_all" on public.enrolments
for select using ( auth.role() = 'authenticated' );

drop policy if exists "enrolments_write_managers" on public.enrolments;
create policy "enrolments_write_managers" on public.enrolments
for insert with check ( auth.role() = 'authenticated' )
for update using ( auth.role() = 'authenticated' )
for delete using (
  exists (select 1 from public.managers m where m.user_id = auth.uid() and m.role in ('creator'))
...
create or replace function public.enrolments_after_ins_upd()
returns trigger language plpgsql as $$
begin
  if NEW.status = 'completed' then
    NEW.needs_refresher := coalesce(NEW.needs_refresher, false);
  end if;
  return NEW;
end;
$$;
...
create trigger trg_enrolments_after
before insert or update on public.enrolments
for each row execute function public.enrolments_after_ins_upd();
